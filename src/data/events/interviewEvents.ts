...
export const interviewEvents: Record<string, GameEvent> = {
  ...
  'interview_onsite_gauntlet_r1': {
    ...
    choices: [
      {
        text: '甩大词轰炸：一致性哈希 / CAP / 最终一致性 / 分层缓存……先用架构黑话镇住场子',
        effect: (s) => {
      ...
      {
        text: '手撕真实架构：老老实实画数据流、估容量、逐层聊取舍',
        effect: (s) => {
  ...
  'interview_onsite_gauntlet_r2': {
    ...
    choices: [
      {
        text: '直接手撕 Hard 最优解，一遍 bug-free',
        effect: (s) => {
      ...
      {
        text: '边写边讲思路，主打沟通与 collaboration',
        effect: (s) => {
  ...
  'interview_onsite_gauntlet_r3': {
    ...
    choices: [
      ...
      {
        text: '真诚讲成长故事，主打 culture fit 与长期主义',
