...
export function migrateSaveData(raw: unknown): MigratedSaveResult {
  ...
  const migratedState: GameState = {
    ...
    has_adu_rented: Boolean(rawState.has_adu_rented),
    parents_helped_house: Boolean(rawState.parents_helped_house),
    investment_properties: Array.isArray(rawState.investment_properties) ? rawState.investment_properties : [],
    rental_income: typeof rawState.rental_income === 'number' && !isNaN(rawState.rental_income) ? Math.max(0, rawState.rental_income) : 0,
    last_housing_action_year: typeof rawState.last_housing_action_year === 'number' ? rawState.last_housing_action_year : 0,
    last_promo_age: typeof rawState.last_promo_age === 'number' ? rawState.last_promo_age : 0,
    fire_tier: typeof rawState.fire_tier === 'string' ? rawState.fire_tier : 'basic',
