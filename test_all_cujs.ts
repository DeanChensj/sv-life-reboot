...
// -----------------------------------------------------------------------------
// CUJ 65: Mid-Year Promotion & Job-Hop Prorated Compensation (50% H1 + 50% H2)
// Verify that:
...
// -----------------------------------------------------------------------------
{
  console.log('--- [CUJ 65] Mid-Year Promotion & Job-Hop Prorated Compensation ---');
  const baseEngineer = {
    ...generateInitialState(nextCujSeed()),
    job_type: 'big_tech',
    company: 'google',
...
// -----------------------------------------------------------------------------
// CUJ 68: Startup Launch & Angel Pitch Gauntlet (Sand Hill Road / YC / Bootstrapping)
// Verify that:
...
// -----------------------------------------------------------------------------
{
  console.log('--- [CUJ 68] Startup Launch & Angel Pitch Gauntlet ---');
...
// -----------------------------------------------------------------------------
// CUJ 69: Unfunded companies cannot scale — you must raise
// Bootstrapping keeps 100% of the equity, and exit pays valuation x equity%, so without a cap
// "never raise" strictly dominated raising ($3435w vs $1050w) and hollowed out the whole
// dilution tradeoff. An un-raised company is hard-capped at $200w; any round lifts it.
// -----------------------------------------------------------------------------
{
  console.log('--- [CUJ 69] Unfunded Valuation Cap ---');
  const founderHub2 = events['founder_annual_strategy'];
  const pmf2 = founderHub2.choices.find(c => c.text.includes('死磕产品'))!;
  const fund2 = founderHub2.choices.find(c => c.text.includes('沙丘路'))!;
  const mkFounder = (equity: number) => ({
    ...generateInitialState(nextCujSeed()),
    job_type: 'startup_founder', founder_stage: 'pre_seed', company_valuation: 180,
    founder_equity_pct: equity, cash: 30, health: 85, leetcode: 60, network: 40, charm: 14,
    age: 32, year: 2030, status: 'playing',
  } as GameState);

  // 1. Bootstrapped (100%) is capped no matter how long it grinds.
  let boot = mkFounder(100);
  for (let y = 0; y < 12; y++) {
    boot = applyStateTransition(boot, pmf2.effect(boot), { eventId: 'founder_annual_strategy' }).nextState;
    boot = { ...boot, year: boot.year + 1, age: boot.age + 1 } as GameState;
  }
  assert((boot.company_valuation || 0) <= 200, `an un-raised company is capped at $200w (got $${boot.company_valuation}w)`);
  assert(boot.founder_stage === 'pre_seed', 'an un-raised company never advances past pre_seed (the stage sync derives from valuation)');

  // 2. A funded founder with the same grind is NOT capped.
  let funded = mkFounder(90);
  for (let y = 0; y < 12; y++) {
    funded = applyStateTransition(funded, pmf2.effect(funded), { eventId: 'founder_annual_strategy' }).nextState;
    funded = { ...funded, year: funded.year + 1, age: funded.age + 1 } as GameState;
  }
  assert((funded.company_valuation || 0) > 200, `a funded company grows past the cap (got $${funded.company_valuation}w)`);

  // 3. Raising lifts the cap permanently (and dilutes).
  let boot2 = mkFounder(100);
  let raised = false;
  for (let k = 0; k < 5000 && !raised; k++) {
    const e = fund2.effect(boot2) as Partial<GameState>;
    if (e.founder_stage === 'seed') {
      boot2 = applyStateTransition(boot2, e, { eventId: 'founder_annual_strategy' }).nextState;
      raised = true;
    }
  }
  assert(raised, 'a bootstrapped founder can reach a seed round');
  assert((boot2.founder_equity_pct || 100) < 100, 'raising dilutes below 100%');
